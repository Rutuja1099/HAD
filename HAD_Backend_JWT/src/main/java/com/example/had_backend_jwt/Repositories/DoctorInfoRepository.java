...
public interface DoctorInfoRepository extends JpaRepository<DoctorInfo,Integer> {
    ...
    Optional<DoctorInfo> findByDrRegNo(String drRegNo);
...
//    Optional<DoctorInfo> findByDrRegNo(@Param("drRegNo") Integer drRegNo);
    ...
    @Query(value = "SELECT drId,drFullName, drSpecialization, drExperience, drGender FROM doctorInfo ORDER BY drExperience DESC ;",nativeQuery = true)
    List<Object[]> SuggestDoctorsListDesc();
}
